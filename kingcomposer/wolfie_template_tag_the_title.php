...
$class_title = array( 'wolfie_title' );

$wrap_class[] = 'wolfie-title-wrap';
...
<div class="<?php echo esc_attr( implode( ' ', $wrap_class ) ); ?>">
	...
	<?php
		...
		echo '<'. $type .' class="'. implode( ' ', $class_title ) . '">';
			if ( !empty( $link_url ) ) {
				echo '<a href="'. esc_url( $link_url ) .'" class="wolfie_title_link" title="'. esc_attr( $link_title ) .'" target="'. esc_attr( $link_target ) .'">'. $text .'</a>';
			} else {
				echo $text;
			}
		echo '</'. $type .'>';
